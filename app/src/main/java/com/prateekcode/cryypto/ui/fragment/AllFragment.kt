...
import com.prateekcode.cryypto.databinding.FragmentAllBinding
...
class AllFragment : BaseFragment() {
    ...
    private val binding: FragmentAllBinding by lazy {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        initCurrencies()
        initShimmerEffect()
        binding.allCoins.rvCoinList.apply {
            init(mContext)
    ...
    private fun initCurrencies() {
        lifecycleScope.launch {
            homeVm.currencyList(CurrencyTrend.ALL).collectLatest {
    ...
    private fun initShimmerEffect() {
        currencyAdapter.addLoadStateListener { loadState ->
            when {
                ...
                loadState.refresh is LoadState.NotLoading && loadState.append.endOfPaginationReached && currencyAdapter.itemCount < 1 -> {
                    binding.apply {
                        shimmerContainer.hide()
                        shimmerContainer.stopShimmer()
                        allCoins.rvCoinList.hide()
